...
@section('content-header')
    <h1>
        {{ trans('order::orders.title.create order') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li><a href="{{ route('admin.order.order.index') }}">{{ trans('order::orders.title.orders') }}</a></li>
        <li class="active">{{ trans('order::orders.title.create order') }}</li>
    </ol>
@stop

@section('content')
    {!! Form::open(['route' => ['admin.order.order.store'], 'method' => 'post']) !!}
    <div class="row">
        <div class="col-md-12">
                ...
                <div class="tab-content">
                    ...
                    @foreach (LaravelLocalization::getSupportedLocales() as $locale => $language)
                        ...
                        <div class="tab-pane {{ locale() == $locale ? 'active' : '' }}" id="tab_{{ $i }}">
                            @include('order::admin.orders.partials.create-fields', ['lang' => $locale])
                        </div>
                    @endforeach
