...
class CountryServiceProvider extends ServiceProvider
{
    ...
    public function register()
    {
        ...
        $this->app['events']->listen(LoadingBackendTranslations::class, function (LoadingBackendTranslations $event) {
            $event->load('countries', array_dot(trans('country::countries')));
            $event->load('quocgias', array_dot(trans('country::quocgias')));
            // append translations
    ...
    private function registerBindings()
    {
        ...
        $this->app->bind(
            'Modules\Country\Repositories\QuocGiaRepository',
            function () {
                $repository = new \Modules\Country\Repositories\Eloquent\EloquentQuocGiaRepository(new \Modules\Country\Entities\QuocGia());

                if (! config('app.cache')) {
                    return $repository;
                }

                return new \Modules\Country\Repositories\Cache\CacheQuocGiaDecorator($repository);
            }
        );
// add bindings
